...
*(Observed in an earlier iteration of the loyalty-points service, prior to the hardened Planner policies. This illustrates a real failure mode the semantic auditor is designed to catch — see Section 11 for the specific Run B trace referenced in this report, which reflects a later, more hardened contract and did not require this retry.)*

During an earlier loyalty service generation, the endpoint generator initially produced code referencing `EarnRequest` and `RedeemRequest` without defining those symbols.
...
The comparison between Run B and Run A tracks pipeline policy hardening across two different tasks. The underlying model checkpoint remained identical between runs; the control scaffolding around it was refined in between.

**Run B: Customer Loyalty Points Service**

- Status: Planner contract for this run passed on the first attempt; all three groups (`models`, `dependencies`, `endpoints`) received Green Light with zero group-level retries.
...
- Known Gap (Duplicate Validation): the Generator reimplemented a manual defensive check (`if points < 0: raise ValueError`) inside the class method despite Pydantic already enforcing the same constraint at the request boundary. The branch is unreachable dead code — harmless in this run, but it demonstrates that structurally valid, audit-passing code can still contain redundant logic no current AST/semantic check flags.
- Verdict: **PASS with Dead Code.** Valid and safe, but highlights redundant logic not yet pruned by static checks.

**Run A: In-Memory User Session Tracker (Hardened Pipeline)**

- Status: Run using an explicit Pydantic-subclass policy and stricter dependency-ordering rules added after the gap above was identified.
- Task Profile: Explicit Pydantic subclass requirement, singleton registry state.
- Execution Telemetry:
  - Planner: 17.93 s
  - models: 6.69 s
  - dependencies: 1.94 s
...
- Verdict: **PASS (0 retries).**

**On the Planner timing difference:** the two Planner durations above (60.73 s vs. 17.93 s) are not a controlled latency benchmark — the tasks differ and each Planner call includes any silent internal JSON-validation retries (the `plan()` retry loop does not log intermediate attempts). The comparison that *is* directly attributable to policy hardening is the retry outcome: earlier, less-constrained contracts required semantic-level retries to converge (Section 5); once the dependency-ordering and Pydantic-subclass policies were made explicit, subsequent runs — including both traces above — converged with zero group-level retries.
...
The goal is not to trust the model more. The goal is to give the system more control over the model.
...
- **Run A Telemetry Trace**: [In-Memory User Session Tracker trace](https://github.com/sylxforce-dev/sovereign-codeswat/blob/main/Logs/Logs_1.md) (~30 s total, 0 retries).
- **Run B Telemetry Trace**: [Customer Loyalty Points Service trace](https://github.com/sylxforce-dev/sovereign-codeswat/blob/main/Logs/Logs_2.md) (~72 s total, 0 group-level retries, dead-code gap noted in Section 11).
